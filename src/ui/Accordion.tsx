...
export type AccordionItem = {
  ...
  // tone='attention' = "지금 펼치기 전에 눈에 띄어야 하는 행" — 좌측 색 띠 + 은은한 틴트. 남발 금지(소수만).
  tone?: 'attention';
  // 틴트 색(토큰 enum만) — 기본 danger(행동요구 색축, CountBadge·Callout과 공유). 의미가 다르면 success/warning/info/neutral.
  //  tone='attention'일 때만 적용. {color}-0 = 배경 틴트 / {color}-6 = 좌측 띠.
  color?: BadgeColor;
};
...
export function Accordion({ items, multiple = false, defaultOpen = [], clearAttentionOnOpen = false }: Props) {
  ...
  // attention 행은 색(기본 danger)을 CSS 역할 변수(--erp-attn-*)로 흘린다 — controls.css가 그걸 읽어 칠한다(토큰값만 통과).
  const attnStyle = (it: AccordionItem): CSSProperties | undefined => {
    if (it.tone !== 'attention') return undefined;
    const c = it.color ?? 'danger';
    return { ['--erp-attn-bg']: `var(--mantine-color-${c}-0)`, ['--erp-attn-band']: `var(--mantine-color-${c}-6)` } as CSSProperties;
  };
