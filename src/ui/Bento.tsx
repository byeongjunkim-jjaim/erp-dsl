...
//    colSpan·rowSpan으로 *선택*한다. 그 선택만이 가변이며, 그 이상의 높이 가변은 이 층에 존재하지 않는다.
//  · 내용이 footprint를 넘치면 위젯이 *내부에서* 처리(스크롤/요약) — Tile은 overflow:hidden으로 셀을 고정한다.
//  · [백로그] 셀 모델(12고정+2span vs columns 자유)·ROW_UNIT 실측·모바일 reflow는 첫 위젯-배치 페이지에서 확정.
//    지금은 행동 보존 — 분류만 템플릿→배치 프리미티브로 바로잡고 columns/colSpan 동작은 그대로.
...
function Bento({ columns = 12, gap = 'lg', children }: GridProps) {
  return (
    <div style={{
      display: 'grid',
